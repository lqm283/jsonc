 ...
 * @LastEditTime : 2022-06-09 15:30:20
...
static const char* jsonc_type_uint8[] = {"uint8_t", "u8", "unsigned char",
                                         "uint8", NULL};
static const char* jsonc_type_int16[] = {"short", "int16_t", "short int",
                                         "int16", "i16",     NULL};
static const char* jsonc_type_uint16[] = {"uint16_t", "u16", "unsigned short",
                                          "unsigned short int", NULL};
static const char* jsonc_type_int32[] = {"int", "int32_t", "int32", "i32",
                                         NULL};
static const char* jsonc_type_uint32[] = {"int",          "uint32_t", "u32",
...
static void* jsonc_change_cbasearr_to_json(char* buf, void* st,
                                           const struct struct_mem* mem);
static void* jsonc_change_cmultarr_to_json(char* buf, void* st,
                                           const struct struct_mem* mem);
static void* jsonc_change_cptrbase_to_json(char* buf, void* st,
                                           const struct struct_mem* mem);
static void* jsonc_change_cptrmult_to_json(char* buf, void* st,
                                           const struct struct_mem* mem);
static void* jsonc_change_cptrbasearr_to_json(char* buf, void* st,
                                              const struct struct_mem* mem);
static void* jsonc_change_cptrmultarr_to_json(char* buf, void* st,
                                              const struct struct_mem* mem);
...
static inline int is_this_char(int dest, int src) { return (dest == src); }
...
static int jsonc_change_string_to_json(void* buf_start, void** buf_end,
                                       const char* str) {
  int ret = 0;
  char* buf = buf_start;
...
static int jsonc_change_num_to_json(void* buf_start, void** buf_end, void* st,
                                    const struct struct_mem* mem) {
  char num[64] = {0};
  char* str = num;
...
static int jsonc_change_bool_to_json(void* buf_start, void** buf_end, void* st,
                                     const struct struct_mem* mem) {
  char buf[8] = {0};
  char* str = buf;
  const char* bool;
...
void* jsonc_change_cbase_to_json(char* buf, void* st,
                                 const struct struct_mem* mem) {
  void* ret = NULL;
...
static void* jsonc_change_cmultarr_to_json(char* buf, void* st,
  ...
  *buf++ = '[';
  for (int i = 0; i < length; i++) {
    buf = jsonc_obj_to_json(buf, st + (i * mem->type_info->length),
                            mem->type_info);
    if ((i + 1) != length) {
      *buf++ = ',';
    }
...
static inline void l__list_add(struct list_head* _new, struct list_head* prev,
  ...
  do {
    ...
    if (ele->mem_addr != NULL) {
      if (ele->type == Obj) {
        ...
        ret =
            jsonc_match_mult(ele->mem_addr, ele->mem.type_info, ele->value.obj);
        if (ret) {
          return ret;
        }
        ...
        do {
          ...
          if (arr_mem->type == Obj) {
            if (!(ele->c_type & cMult)) {
              ...
              ret = jsonc_match_mult(arr_mem->mem_addr, arr_mem->mem.type_info,
                                     arr_mem->value.obj);
              if (ret) {
                return ret;
              }
